...
class HomoAttentionAggregationLayer(nn.Module):
    def __init__(
        ...
        self.attn_e = nn.Parameter(
            torch.randn(1, val_feats, dtype=torch.float), requires_grad=True
        )
    ...
    def forward(self, g, ft_q, ft_k, ft_e=None, return_ev=False):
        ...
        if ft_e is not None:
            e = e + ft_e
        e = (self.attn_e * torch.sigmoid(e)).view(-1, self.num_heads, self.head_feats).sum(
            -1, keepdim=True
        )
        if return_ev:
            return e, v
    ...
    def forward(self, g, ft_src):
        ...
        for vtype, eutypes in self.vtype2eutypes.items():
            ...
            for etype, utype in eutypes:
                ...
                src_val.append(v)
                attn_score.append(e)
            src_nid = torch.cat(src_nid, dim=0)
            dst_nid = torch.cat(dst_nid, dim=0)
            ...
            src_val = torch.cat(src_val, dim=0)  # (num_utypes_nodes, num_heads, num_feats)
            attn_score = torch.cat(attn_score, dim=0)  # (num_edges, num_heads, 1)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.user_indices = nn.Parameter(
            torch.arange(num_users, dtype=torch.long), requires_grad=False
        )
